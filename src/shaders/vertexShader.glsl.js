...
float fit(float unscaled,float originalMin,float originalMax,float minAllowed,float maxAllowed){
	return (unscaled-originalMin)*(maxAllowed-minAllowed)/(originalMax-originalMin)+minAllowed;
}

float wave(vec3 position){
	float color=fit(smoothMod(position.y*6.0,1.0,1.5),0.34,0.6,0.0,1.0);
	return color;
}
void main() {
	vPosition=position;
	...
	float finalPattern=wave(noisePattern);
	vDisplacement=finalPattern;

	float displacement=vDisplacement/3.0;
	vec3 finalPosition=position+normal*displacement;
  float distanceFromMouse=distance(uMouse,vPosition);
  if(distanceFromMouse<0.4){
    finalPosition=position+(normal*clamp(distanceFromMouse,0.0,0.2))*displacement;
  }
